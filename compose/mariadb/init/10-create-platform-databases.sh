...
[ "$MARIADB_MCP_READER_USER" != "$MARIADB_USER" ] || {
  printf '%s\n' 'MARIADB_MCP_READER_USER must differ from MARIADB_USER.' >&2
  exit 1
}

sql_user=$(printf '%s' "$MARIADB_USER" | sed "s/'/''/g")
sql_password=$(printf '%s' "$MARIADB_PASSWORD" | sed "s/'/''/g")
sql_mcp_user=$(printf '%s' "$MARIADB_MCP_READER_USER" | sed "s/'/''/g")
sql_mcp_password=$(printf '%s' "$MARIADB_MCP_READER_PASSWORD" | sed "s/'/''/g")
...
CREATE USER IF NOT EXISTS '$sql_mcp_user'@'%' IDENTIFIED BY '$sql_mcp_password';
GRANT SELECT ON cycling_platform_silver.* TO '$sql_mcp_user'@'%';
SQL
