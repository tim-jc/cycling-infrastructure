...
static_required_keys=(MARIADB_USER MARIADB_PASSWORD MARIADB_MCP_READER_USER MARIADB_MCP_READER_PASSWORD MARIADB_GRAFANA_READER_USER MARIADB_GRAFANA_READER_PASSWORD MARIADB_ROOT_PASSWORD MARIADB_PORT GRAFANA_BIND_ADDRESS GRAFANA_PORT GRAFANA_ADMIN_USER GRAFANA_ADMIN_PASSWORD STRAVA_CLIENT_ID STRAVA_CLIENT_SECRET GOOGLE_HEALTH_CLIENT_ID GOOGLE_HEALTH_CLIENT_SECRET NTFY_TOPIC CYCLING_ANALYTICS_NTFY_TOPIC)
...
static_validate_plaintext() {
  ...
  [[ "$(static_value MARIADB_MCP_READER_USER "$file")" != "$(static_value MARIADB_USER "$file")" ]] ||
    static_fail 'MARIADB_MCP_READER_USER must be distinct from MARIADB_USER.'
  [[ "$(static_value MARIADB_GRAFANA_READER_USER "$file")" != "$(static_value MARIADB_USER "$file")" ]] ||
    static_fail 'MARIADB_GRAFANA_READER_USER must be distinct from MARIADB_USER.'
  [[ "$(static_value MARIADB_GRAFANA_READER_USER "$file")" != "$(static_value MARIADB_MCP_READER_USER "$file")" ]] ||
    static_fail 'MARIADB_GRAFANA_READER_USER must be distinct from MARIADB_MCP_READER_USER.'
