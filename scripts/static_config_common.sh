...
static_required_keys=(MARIADB_USER MARIADB_PASSWORD MARIADB_ROOT_PASSWORD MARIADB_PORT STRAVA_CLIENT_ID STRAVA_CLIENT_SECRET GOOGLE_HEALTH_CLIENT_ID GOOGLE_HEALTH_CLIENT_SECRET NTFY_TOPIC)
static_forbidden_keys=(STRAVA_REFRESH_TOKEN GOOGLE_HEALTH_REFRESH_TOKEN CYCLING_PLATFORM_EXECUTION_HOST CYCLING_PLATFORM_RUNTIME_UID CYCLING_PLATFORM_RUNTIME_GID)
...
static_metadata_digest() {
  ...
  grep -Fxq 'format=cycling-static-compose-age-v1' "$file" || static_fail 'Static configuration metadata format is unsupported.'
