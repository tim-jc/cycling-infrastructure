...
AGE_BIN="${AGE_BIN:-age}"; RECIPIENT=""; IDENTITY=""; SOURCE=""; OUTPUT=""; TMP=""
usage(){ printf '%s\n' 'Usage: backup_static_config.sh --source compose/.env --recipient AGE_RECIPIENT --identity FILE --output FILE.age'; }
trap '[[ -z "$TMP" ]] || rm -rf -- "$TMP"' EXIT
while (( $# )); do case "$1" in --source) SOURCE="${2:-}"; shift 2;; --recipient) RECIPIENT="${2:-}"; shift 2;; --identity) IDENTITY="${2:-}"; shift 2;; --output) OUTPUT="${2:-}"; shift 2;; -h|--help) usage; exit 0;; *) static_fail "Unknown argument: $1";; esac; done
...
printf 'format=cycling-static-compose-age-v1\nbackup_completed_at_utc=%s\nciphertext_file=%s\nciphertext_sha256=%s\n' "$completed" "$(basename "$OUTPUT")" "$digest" >"$metadata"; chmod 600 "$metadata"
